...
| 列名        | 数值类型      | 可否为空 | 默认值            | 备注                             |
| :---------- | :------------ | :------- | :---------------- | :------------------------------- |
| id          | bigint(20)    | NOT NULL | 自增              | 用户 id，主键                    |
| username    | varchar(256)  | NOT NULL | 无                | 账号                             |
...
| roles       | varchar(256)  | NULL     | ["user"]          | 用户角色：user/admin/ban         |
...
| 列名        | 数值类型      | 可否为空 | 默认值            | 备注                             |
| :---------- | :------------ | :------- | :---------------- | :------------------------------- |
| id          | bigint(20)    | NOT NULL | 自增              | 题库 id，主键                    |
| title       | varchar(256)  | NOT NULL | 无                | 标题                             |
| description | text          | NULL     | 无                | 描述                             |
| picture     | varchar(2048) | NULL     | 无                | 图片                             |
| del_flag    | char(1)       | NULL     | '0'               | 删除标志（0代表存在，2代表删除） |
| create_by   | bigint(20)    | NULL     | NULL              | 创建者                           |
| create_time | datetime      | NULL     | current_timestamp | 创建时间                         |
| update_by   | bigint(20)    | NULL     | NULL              | 更新者                           |
...
| 列名        | 数值类型      | 可否为空 | 默认值            | 备注                             |
| :---------- | :------------ | :------- | :---------------- | :------------------------------- |
| id          | bigint(20)    | NOT NULL | 自增              | 题目 id，主键                    |
| title       | verchar(256)  | NOT NULL | 无                | 标题                             |
| content     | text          | NULL     | NULL              | 内容                             |
| tags        | varchar(1024) | NULL     | NULL              | 标签列表（JSON数组）             |
...
create table if not exists `question` (
  ...
  auto_increment = 100 comment '题目题表';
...
# 题目与题库关联表(question_bank_question)

| 列名             | 数值类型   | 可否为空 | 默认值 | 备注                          |
| :--------------- | :--------- | :------- | :----- | :---------------------------- |
| question_bank_id | bigint(20) | NOT NULL | 无     | 题库 id（外键，关联题库表id） |
| question_id      | bigint(20) | NOT NULL | 无     | 题目 id（外键，关联题目表id） |
