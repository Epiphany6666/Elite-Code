...
import cn.elitecode.module.member.dal.dataobject.user.MemberUser;
...
public class MemberUserController {
    ...
    @GetMapping("/{userId}")
    private CommonResult<MemberUser> getUser(@PathVariable Long userId) {
        MemberUser memberUser = memberUserService.getUserById(userId);
        return CommonResult.success(memberUser);
    }

    @ApiOperation(value = "根据分页条件查询会员信息")
    @PostMapping("/list")
    private CommonResult<CommonPage<MemberUser>> list(@RequestBody MemberUserQueryReqVO reqVO) {
